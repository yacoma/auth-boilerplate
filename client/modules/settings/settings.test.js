...
test.serial('should change nickname', t => {
  ...
  return cerebral.runSignal('settings.profileFormSubmitted')
    .then(({state}) => ([
      t.is(state.user.nickname, 'NewTest')
    ]))
})
...
test.serial('should change email', t => {
  ...
  return cerebral.runSignal('settings.emailFormSubmitted')
    .then(({state}) => ([
      t.is(state.user.email, 'new-test@example.com')
    ]))
})
