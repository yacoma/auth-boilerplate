...
test.serial('should fetch users', t => {
  ...
  return runSignal(fetchUsers, cerebral)
    .then(({state}) => [
      ...
      t.is(state.admin.users[Object.keys(state.admin.users)[0]]['@id'], '/users/1'),
      t.is(state.admin.users[Object.keys(state.admin.users)[0]]['nickname'], 'Leader'),
      t.is(state.admin.users[Object.keys(state.admin.users)[0]]['email'], 'leader@example.com'),
      t.is(state.admin.users[Object.keys(state.admin.users)[0]]['emailConfirmed'], false),
      t.is(state.admin.users[Object.keys(state.admin.users)[0]]['isAdmin'], true),
      t.is(state.admin.users[Object.keys(state.admin.users)[0]]['registerIP'], '')
    ])
})
...
test.serial('should return noUsersFound', t => {
  ...
  return runSignal(fetchUsers, cerebral, {recordActions: 'byName'})
    .then(({mergeUsers}) => [
      t.true(mergeUsers.output.noUsersFound)
    ])
})
