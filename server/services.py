...
class MailerService:
    def __init__(
        self, username, password, host, port, starttls, ssl, skip_login, token_service
    ):
    ...
    def send_confirmation_email(self, user, request):
        token = self.token_service.create(user.email, "email-confirmation-salt")

        confirm_url = request.application_url + request.class_link(
            ConfirmEmail, variables={"id": user.id, "token": token}
        )
    ...
    def send_reset_email(self, user, request):
        token = self.token_service.create(user.email, "password-reset-salt")

        reset_url = request.application_url + request.class_link(
            ResetPassword, variables={"id": user.id, "token": token}
        )
