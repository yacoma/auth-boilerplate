...
def root_default(self, request):
    return {
        "collections": {
            "users": {"@id": request.class_link(UserCollection)},
            "groups": {"@id": request.class_link(GroupCollection)},
        }
    }
...
def user_collection_get(self, request):
    user_collection = {"users": [request.view(user) for user in self.query()]}
    if self.page != 0:
...
def group_collection_get(self, request):
    return {"groups": [request.view(group) for group in self.query()]}
...
def group_collection_add(self, request, json):
    ...
    if not Group.exists(name=name):
        ...
        return {"@id": request.class_link(Group, variables={"id": group.id})}
        ...
        return {"validationError": "Group already exists"}


@App.json(
    model=Group, request_method="PUT", load=group_validator, permission=EditPermission
)
...
def confirm_email(self, request):
    ...
    flash_encoded = (
        urlsafe_b64encode(flash.encode("utf-8")).replace(b"=", b"").decode("utf-8")
    )
...
@App.json(model=SendResetEmail, request_method="POST", load=send_reset_email_validator)
...
def request_reset_password(self, request):
    ...
    if token_valid and user.email_confirmed:
        path = "/newpassword"
        query_dict = {
            "@id": request.class_link(
                ResetPassword, variables={"id": self.id, "token": self.token}
            )
        }
...
@App.json(model=ResetPassword, request_method="PUT", load=reset_password_validator)
