...
def test_login():
    ...
    response = c.post(
        "/login", json.dumps({"email": "mary@EXAMPLE.COM", "password": "test2"})
    )
...
def test_add_user(smtp):
    ...
    new_user_json = json.dumps(
        {"nickname": "NewUser", "email": "newuser@example.com", "password": "test7"}
    )
    ...
    new_user_json = json.dumps(
        {"nickname": "NewUser", "email": "newuser@example", "password": "test9"}
    )
...
def test_send_reset_email(smtp):
    ...
    response = c.post(
        "/reset", json.dumps({"email": "not_exist@example.com"}), status=403
    )
    assert response.json == {"validationError": "Email not found"}

    response = c.post(
        "/reset", json.dumps({"email": "newuser@example.com"}), status=403
    )
    ...
    response = c.post("/reset", json.dumps({"email": "newuser@example.com"}))
